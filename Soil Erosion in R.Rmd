...
```{r}
# load data 
rnf_sj <- raster("Data/Rainfall/PRISM/res_rnf_sj.tif")
plot(rnf_sj)
...
# save raster
raster::writeRaster(r_moore, filename = file.path("Data/Rainfall/PRISM//erosivity_factor.tif"), format="GTiff", overwrite=TRUE)
```
...
```{r}
ef <- raster("Data/Erodibility/drive-download-20221024T195607Z-001/SJ_Erodibility.tif")
raster::plot(ef)
```
...
```{r, eval=FALSE, include=TRUE}
# flow accumulation
flow_acc <- raster("Data/DEM or DSM/FlowAcc_sj_1.tif/FlowAcc_sj_1.tif")

plot(flow_acc)
plot(st_geometry(sj), add = TRUE)

# crop
crop_flow_acc <- crop(flow_acc, extent(sj))
mask_flow_acc <- mask(crop_flow_acc, sj)
plot(mask_flow_acc)

# resampling/interpolation
target <- slope
res(target) <- res(slope)
mask_flow_acc <- raster::resample(mask_flow_acc, target, method = "bilinear")
plot(mask_flow_acc)
```


```{r, eval=FALSE, include=TRUE}
p1 <- (((mask_flow_acc*9.28) / 22.1)^0.4)
p2 <- ((sin(slope*0.01745)/0.09)^1.4)
...
#LS <- (((mask_flow_acc*9.28) / 22.1)^0.4) * ((sin(slope*0.01745)/0.09)^1.4) * 1.4


#Power("FlowAcc_sj" * 9.28 / 22.1, 0.4) * Power(Sin("sj_slope.tif"* 0.01745) / 0.09, 1.4) * 1.4
```
...
```{r}
# load LS factor
ls_factor <- raster("Data/LS /LS_factor.tif")
plot(ls_factor)
```
...
```{r, eval=FALSE, include=TRUE}
LS = (0.065 + 0.0456*(slope) + 0.006541*(slope)^2)*(slope length/22.1)^NN
```

Esri community; To calculate the LS factor for the RUSLE equation, first calculate flow accumulation (facc) and slope in degrees (slp). Then a bit of map algebra in the raster calculator yields LS.
...
```{r}
cs <- raster("Data/Crop Cover/drive-download-20221025T182147Z-001/Reclassify_SJ_Cropcover_2021.tif")
plot(cs)
...
- Ctillage = 1 for conventional Strip cropping;
- Ctillage = 0.35 for conservation contour cropping; 
- Ctillage = 0.25 for terrace cropping.
...
# save 
raster::writeRaster(RUSLE_1, filename = file.path("RUSLE_1.tif"), format="GTiff", overwrite=TRUE)
```
...
# save 
raster::writeRaster(RUSLE_2, filename = file.path("RUSLE_2.tif"), format="GTiff", overwrite=TRUE)
```
...
# save 
raster::writeRaster(RUSLE_3, filename = file.path("RUSLE_3.tif"), format="GTiff", overwrite=TRUE)
```
